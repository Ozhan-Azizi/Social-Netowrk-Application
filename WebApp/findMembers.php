...
<html>
    ...
    if(isset($_GET['startWatching']))
    {
        $watching = $_GET['startWatching'];
        $result = queryMysql("SELECT * FROM friends WHERE user='$watching' AND friend='$user'");
        if($result->num_rows)
        {
        ...
        {
            queryMysql("INSERT INTO friends VALUES ('$watching', '$user')");
        }
    }

    if(isset($_GET['stopWatching']))
    {
        $watching = $_GET['stopWatching'];
        queryMysql("DELETE FROM friends WHERE user='$watching' AND friend='$user'");
    }
...
?>
            ...
            <div class='inputEverything'>
                <div id='selection'>
                <form method ='get' action='findMembers.php?checkBox'>
                <input id="checkbox" name="checkBox" type="checkbox" value="1" />All Members
                <input id="checkbox" name="checkBox" type="checkbox" value="0" />Recommended
                <br>
...
<?php
            if($check == 0)
            {
                displayMembersRecommended($user);
            }
            elseif($check == 1)
            {
