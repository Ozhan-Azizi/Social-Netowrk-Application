...
if($_POST['title'])
{
    $title1 = sanitizeString($_POST['title']);
    $type = $_POST['checkPost'];
    $result = queryMysql("SELECT * FROM overChat WHERE title='$title1'");

    if($result->num_rows)
    {
        $error = "That chat room already exist.<br><br>";
    }
    else
    {
        $error = "Chat room Created";
        queryMysql("INSERT INTO overChat VALUES('0', '$title1', '$type', '$user')");
    }
...
<html>
    <head>
        ...
        <link type='text/css' rel='stylesheet' href='allChatRooms.css'/>  
        <link rel='stylesheet' href='jquery-ui.min.css'>
        <link href='http://fonts.googleapis.com/css?family=Oswald:400,300' rel='stylesheet'>
        <link href='http://s3.amazonaws.com/codecademy-content/courses/ltp2/css/bootstrap.min.css' rel='stylesheet'>
        <link rel="alternate" type="application/rss+xml" title="RSS" href="http://www.csszengarden.com/zengarden.xml">
...
<?php
    echo <<<_END
        <div class='MakeRoom'>
            ...
            <form method='post'>
                Title: <input type='text' name='title' value='$title'>$error<br>
                <input id="checkbox" name="checkPost" type="checkbox" value="Public"/>Public<br>
                <input id="checkbox" name="checkPost" type="checkbox" value="Private"/>Private<br>
                <input type='submit' name='submit' value='Create Chat' style='float:right'>
            </form>
        </div>
_END;
?>
...
<?php
        ...
        elseif($displayPrivate == 1)
        {
            displayGroupChatRooms($gtitle);
        }
?>
